...
#define MAX_HEALTH_C 10.0f //Heath of two_characters
...
//Game information
vector<float> music_beats;
ifstream beats_info;
int Max_Health_E; //Enemy_health
...
void UpdateBoxes(float current_time) {
	...
	for (size_t i = 0; i < music_elements_P1.size(); i++) {
		...
		if (music_elements_P1[i].dimension[1] < -2.5f) {
		...
		else if (music_elements_P1[i].dimension[1] < -1.8f) {
	...
	for (size_t i = 0; i < music_elements_P2.size(); i++) {
		...
		if (music_elements_P2[i].dimension[1] < -2.5f) {
			Mix_PlayChannel(-1, miss, 0);
		...
		else if (music_elements_P2[i].dimension[1] < -1.8f) {
...
void ScoreTrack() {
	...
	modelMatrix.Translate(0.25f, -1.0f, 0.0f);
	program->setModelMatrix(modelMatrix);
	DrawText(program, "MISS:" + to_string(not_hit_track), 1.0f, -0.7f);
}
...
//Load Beats
void load_beats(const string& level) {
	beats_info.open(level);
	string line;
	int total_hit = 0;
	while (getline(beats_info, line)) {
		music_beats.push_back(atof(line.c_str()));
		total_hit += 1;
	}
	Max_Health_E = total_hit;
	beats_info.close();
}
...
void play() {
	...
	case 0:
		break;
	case 1:
		load_beats("Level_1.txt");
		challenge = Mix_LoadMUS("Level_1.mp3");
		break;
	case 2:
		load_beats("Level_2.txt");
		challenge = Mix_LoadMUS("Level_2.mp3");
		break;
	case 3:
		load_beats("Level_3.txt");
...
bool on_beat_release(float progress) {
	return progress >= music_beats[nth_beat];
}
